...
| **G2** | **Minimax M3** 🟢 / **Kimi** 🟢 | Proto-Elamite | CDLI ledger entropy | 🟢 **merged** (synth) + 🟢 **G2-REAL landing** — `--multi-fetch` CDLI HTTP; 20 Susa IDs; live probe **FETCH_BLOCKED** (cdli.earth HTTP 500 / legacy 404; no fabrication). 39/39 tests. `feat/g2-real-cdli-fetch-clean` |
...
| **ATLAS** | **Cursor** 🟢 | Language Entropy Atlas + anomaly schema | entropy_atlas + anomalies.json | 🟢 **merged PR #37** — populated from 25 `outputs/*/run.json`; `data/catalog/{entropy_atlas,anomaly_schema}.json`; `tools/scripts/atlas_query.py` (list/find/get); 6/6 tests. Does not upgrade verdicts. `feat/atlas-populate` |
...
| **G22** | **Kimi** 🟢 | Nazca line_detect | synthetic tiles + desert nulls; CLAHE→Sobel→HoughLinesP | 🟢 **landing** — `tools/geo/nazca_line_detect.py`, `data/nazca/`, `outputs/nazca/`. Verdict **FPR_CALIBRATED** (power 100%, FPR_combined 1.67%). Real: **FIXTURE_ONLY** (no Sentinel-2). Figurative reliefs UNDERDETERMINED at 10 m. 15/15 tests. `feat/g22-nazca-line-detect-clean` |
