...
| **B11** | **Cursor** | Cipher negative control | Native Caesar/IC (Decipher needs Rust bootstrap) | 🟢 no English cipher on Crabwood/Chilbolton || **R1** | Minimax | `radio_probe` scaffold | FFT/autocorr/epoch-fold + Wow blocked + FRB 180916 plant | 🟢 scaffold 17/37 → now 37/37 tests. **R1+ real-data path landed:** `tools/radio/chime_frb_fetcher.py` + `frb_real_sources.py` + `--frb-180916-real` CLI flag + `--bundled-mjd-json` override; honest fetch failure surfacing (NEVER fabricates). `outputs/radio/radio_real_stub.json` + `radio_real_notes.md` produced with `fetch_status=UNREACHABLE/PARK` + 🟡 BANNER. **Vela positive-control anchor landed:** `tools/radio/pulsar_fetcher.py` + `--pulsar-vela/synthetic/real` flags + `--bundled-pulsar-csv` override + lab-motto anchor (periodicity = necessary, NOT sufficient for artificiality). Synthetic-vela path recovers P0 = 0.089328385 s ± ~5 microseconds; canonical Manchester 2005 bibcode `2005AJ....129.1993M`. Real-data path surfaces honest fetch failure (all canonical ATNF/Parkes endpoints return HTML parking or 404 as of probe). **R1 polish landed:** `write_notes_markdown()` extended with Vela SYNTHETIC + Vela REAL-DATA (🟡 BANNER on fetch failure, mirror of FRB real-data style) sections; `data/radio/README.md` Vela section with Manchester+2005 PMID/DOI, glitch provenance, CLI summary, and `No fabricated MJDs. Ever.` motto banner. |
...
| **G3** | **Minimax** | Wow! horn beam-fit | Fit sidereal transit to 6EQUJ5 intensities; underdetermined OK | 🟢 **landed** — `radio_probe.py` extended with `synth_wow_beam_transit` + `fit_wow_beam_transit` (pure-numpy 51-step coarse grid search; both Gaussian and sinc fits; r² reported against constant baseline; degeneracy-pair surfaced; underdetermined caveat enforced at N=6-K=3 DOF) + `wow_beam_scramble_null` (24-permutation median+p5/p95 baseline) + `run_wow_beam_fit(mode='synthetic'|'real')` orchestrator + `_wow_beam_motto_stance()` helper; analyze() dispatcher for `wow_beam_fit_synthetic` / `wow_beam_fit_real` / `wow_beam_fit` modes; main() flags `--wow-beam-fit / --wow-beam-fit-synthetic / --wow-beam-fit-real`; `write_notes_markdown()` renders the '## Wow! (1977) — SIDEREAL TRANSIT fit (underdetermined)' section with r² for all 3 fits + recovered params + scramble-null summary + PHL@UPR 2024 cite; 5 new tests (45 → 50); synthetic recovery_pass=True with `|err_mu| <= 0.5 idx && |err_sigma| <= 0.5 idx && |err_amp| <= 1e-6`; real-data path uses Ehman's handwritten 6EQUJ5 transcript; NO scipy dependency; lab motto enforced end-to-end (PHL@UPR 2024 arXiv:2408.08513, CC BY 4.0 attributed Wow! to a hydrogen cloud near a solar-type star -- NATURAL mechanism, NOT ET). |
...
| **G7** | **Opencode** 🟢 | Gorafe megalith landscape | CC BY CSV JOAD 2023 — orientation + spatial null in same DEM box | 🟢 **landed** — `tools/geo/gorafe_probe.py`, `outputs/gorafe/run.json` + `NOTES.md`. Orientation: E/SE dominant (85/108, R=0.82, p≈0). Corridor mean 117° near Dec solstice (Δ=7°) but generic SE bias cannot be ruled out. Verdict: ORIENTATION_STRUCTURE, astronomical alignment UNDERDETERMINED. || **R1++** | Hyper radio (after viz) | CHIME/FRB Catalog 2 periods | Recover 16.35 d on 180916; scramble null on Cat 2 repeaters | ⬜ Kimi complement |
| **G-BLC1** | **Minimax** | `blc1_fetcher` + RFI known-answer | Comb detection on synthetic clock-oscillator intermodulation plant; positive control = RFI hit at Parkes known-RFI freqs (440 / 1217 MHz); scrambled-null drops hits; **live TB fetch DISABLED per 'no TB mirror' brief** | 🟢 **landed** — `tools/radio/blc1_fetcher.py` with bundled-override + NEVER_ATTEMPTED default + test_force hooks (FETCHED synthesises 5-peak positive-control set); `radio_probe.py` extended with `synth_blc1_comb_peaks` + `_blc1_run_peak_detector` + `run_blc1_synthetic` + `run_blc1_real` + `_blc1_module_missing` + `_blc1_motto_stance(_with_recovered)`; analyze() dispatcher for `blc1_synthetic`/`blc1_real`/`blc1` modes; main() flags `--blc1 / --blc1-synthetic / --blc1-real / --bundled-blc1-csv` + post-analyze force_status re-run block; `write_notes_markdown()` renders BLC1 SYNTHETIC + REAL-DATA sections with 🟡 YELLOW BANNER on honest-empty fallback; 8 new tests (37 → 45 total); `data/radio/README.md` G-BLC1 section with Sheikh 2021 (DOI 10.1038/s41550-021-01508-8) cite + 5 admin URLs documented as intentionally-not-contacted; Sheikh 2021 drift correction −0.26 Hz/s (NOT +0.038 Hz/s); PARKES_KNOWN_RFI_FREQS_MHZ positive control list (440/715/982.002/1217/1616 MHz); CC BY 4.0 license for bundled override; NO fabricated peak lists; lab motto enforced end-to-end (periodicity/comb-hit IS NECESSARY, NOT SUFFICIENT FOR ARTIFICIALITY). |
