...
| **N4++** | **Opencode** | Astro caveats harden | BCE equinox labels + real axis-Δaz null | 🟢 **merged PR #10** — validated solar lon; axis Δaz (Ruggles/Šprajc); no alignment signal |
...
| **G19** | **Freebuff** 🟢 | Long Delayed Echoes historic series | radio/FFT; digitized delays | 🟢 **landed** — `tools/scripts/lde_probe.py`, `data/radio/lde/README.md`, `outputs/lde/{run.json,NOTES.md}`. 6 series digitized from Størmer (1928), van der Pol (1928), Appleton (1934), Crawford (1970). Noise-only for individual series (|z|<2.0); combined aggregation z=4.09 (repetition artifact). Verdict: **UNDERDETERMINED \| SHORT_SERIES_WARNING_2_OF_6_N_LT_5**. Lunan CUT = structure not tested. 22/22 tests. `feat/g19-lde` |
| **G20** | queue | Boyajian Star TESS epoch-fold | astro; TIC 272172248 | ⬜ underdetermined |
| **G21** | **Freebuff** 🟢 | Fine-structure α directional variation | astro; King+2012 quasar Δα/α dipole re-run CLEAN | 🟢 **landed** — `tools/scripts/alpha_variation_probe.py`, `data/astro/alpha_variation/{README.md,catalog.json,king_2012_vlt_keck.dat}`, `outputs/alpha_variation/{run.json,NOTES.md,dipole_plot.png,telescope_split.json}`. Brute-force dipole search (72×36 grid, 500 nulls) on 293 clean absorbers; independent recovery within 0.2σ of King+2012. **Per-telescope split (added 2026-07-26):** dipole is VLT-driven — Keck-only z=−0.44, VLT-only z=+2.11. VLT bootstrap resample null z=−0.27 (dipole disappears against telescope-internal correlations). Cross-telescope consistency poor (25.9° separation). Instrument-systematics concern strengthened. Best-fit: RA=260°, Dec=−59°, A=0.974×10⁻⁵. Verdict: **INSTRUMENT_SYSTEMATICS_NULL_NOT_REJECTED \| UNDERDETERMINED \| BEST_FIT_NEAR_KNOWN_DIPOLE \| STRONG_NULL_SEPARATION_2OF4**. 24/24 tests. `feat/alpha-variation` |
...
### N4++ — Opencode (🟢 MERGED PR #10 2026-07-25) — Astro caveats harden
> Landed: validated_solar_lon_deg; calendar_label_unreliable for year<-2000;
> axis Δaz (Stonehenge/Chichén cited); Göbekli/Giza azimuth-only; no alignment signal.
> 20/20 tests. Idle until Captain assigns next.
...
### G11 — Opencode (🟢 MERGED PR #11 2026-07-25) — Cypro-Minoan allography / media
> Landed: Corazza 2022 / sign2vec_d corpus; `tools/scripts/cypro_minoan_probe.py` +
> `ingest_cm_corpus.py`; `outputs/cypro_minoan/{run.json,NOTES.md}`; 13/13 tests.
> Verdict: **STRUCTURE_SIGNAL | MEDIA_DRIVEN_ALLOGRAPHY** (full z≈−28.95; tablet↔other
> Jaccard≈0.53; KA z≈−19.3 PASS; NC z≈0.6 PASS). CM1/2/3 labels unavailable in open
> data — site/medium proxies. Structure ≠ message.
>
> **Later G11+ (not this ticket):** Unicode 16.0 CM block + UCL 3D sign DB.
