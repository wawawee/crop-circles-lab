...
| **ATLAS** | **Cursor** 🟢 | Language Entropy Atlas + anomaly schema | entropy_atlas + anomalies.json | 🟢 **landing** — populated from 25 `outputs/*/run.json`; `data/catalog/{entropy_atlas,anomaly_schema}.json`; `tools/scripts/atlas_query.py` (list/find/get); 6/6 tests. Does not upgrade verdicts. `feat/atlas-populate` |
